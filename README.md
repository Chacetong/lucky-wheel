...
- 「随机权重」一键随机全员权重，范围可调（0.1–100，含两端），数字先快后慢插值到位，转盘扇区面积同步连续变化
- 按权重计算中奖概率，权重统一固定一位小数展示
...
**通用**
- 使用 `localStorage` 自动保存阵容、组数、模式、随机权重范围等
- 全键盘可达（Space / Enter 启动，Tab 焦点循环，ESC 取消 / 关闭）
- 隐藏指令：焦点不在输入框时依次敲 `A` `C` `D` `C`（不区分大小写）载入 ACDC 预设阵容
